...
impl CurrentStatesRepository {
    ...
    pub async fn find_by_id(db: &DbConn, current_state_id: Uuid) -> Result<Option<current_states::Model>, AppError> {
    ...
    pub async fn create(db: &DbConn, current_state_data: current_states::ActiveModel) -> Result<current_states::Model, AppError> {
        current_state_data
            .insert(db)
            .await
            .map_err(AppError::from)
