...
impl DesiredStatesRepository {
    ...
    pub async fn find_by_id(db: &DbConn, desired_state_id: Uuid) -> Result<Option<desired_states::Model>, AppError> {
    ...
    pub async fn create(db: &DbConn, desired_state_data: desired_states::ActiveModel) -> Result<desired_states::Model, AppError> {
        desired_state_data
            .insert(db)
            .await
            .map_err(AppError::DatabaseError)
