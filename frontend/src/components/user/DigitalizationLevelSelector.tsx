...
const DigitalizationLevelSelector: React.FC<
  DigitalizationLevelSelectorProps
> = ({
  label,
  onValueChange,
  initialCurrent = "",
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <Label htmlFor={`${label}-current`} className="mb-2 block">
            Current State
          </Label>
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              {levels.map((level) => (
                <DropdownMenuItem
                  key={level.value}
                  onSelect={() => setCurrentLevel(level.value)}
                >
                  {level.label}
                </DropdownMenuItem>
              ))}
        ...
        <div>
          <Label htmlFor={`${label}-tobe`} className="mb-2 block">
            To-Be State
          </Label>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="w-full justify-between">
                {toBeLevel
                  ? levels.find((l) => l.value === toBeLevel)?.label
                  : "Select to-be level"}
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent>
              {levels.map((level) => (
                <DropdownMenuItem
                  key={level.value}
                  onSelect={() => setToBeLevel(level.value)}
                >
                  {level.label}
                </DropdownMenuItem>
              ))}
      ...
      <div>
        <Label htmlFor={`${label}-comment`} className="mb-2 block">
          Comments
        </Label>
        <Textarea
          id={`${label}-comment`}
