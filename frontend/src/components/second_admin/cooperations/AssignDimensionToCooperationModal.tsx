...
interface AssignDimensionToCooperationModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const AssignDimensionToCooperationModal: React.FC<
  AssignDimensionToCooperationModalProps
> = ({ isOpen, onClose, dimensions, cooperationName }) => {
  ...
  const handleSave = () => {
    // TODO: Implement the logic to save the assigned dimensions for the cooperation
    console.log(
      `Saving dimensions for ${cooperationName}:`,
      selectedDimensions,
    );
    onClose();
  };
        ...
        <div className="space-y-4">
          {dimensions.map((dimension) => (
            <div key={dimension.id} className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor={`dimension-${dimension.id}`}>
                {dimension.name}
              </Label>
            </div>
          ))}
