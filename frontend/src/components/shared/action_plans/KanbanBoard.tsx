...
export function KanbanBoard({ submissionId }: KanbanBoardProps) {
  ...
  const canEdit = roles.includes(ROLES.ORG_ADMIN) || roles.includes(ROLES.COOP_ADMIN);
  ...
  const columns = {
    ...
    in_progress: actionPlan.action_items.filter((item) => item.status === "in_progress"),
    done: actionPlan.action_items.filter((item) => item.status === "done"),
    approved: actionPlan.action_items.filter((item) => item.status === "approved"),
  };

  return (
    <div
      className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 h-full"
    >
      {/* To Do */}
      <div className="bg-gray-50/80 rounded-xl border border-gray-200 flex flex-col overflow-hidden h-full">
        <div className="p-4 border-b border-gray-200 bg-white/50 rounded-t-xl shrink-0">
          ...
          <div className="h-1 w-full bg-gray-100 rounded-full mt-3"><div className="h-full bg-gray-400 w-full rounded-full opacity-50" /></div>
        </div>
      ...
      {/* In Progress */}
      <div className="bg-blue-50/50 rounded-xl border border-blue-100 flex flex-col overflow-hidden h-full">
        <div className="p-4 border-b border-blue-100 bg-white/50 rounded-t-xl shrink-0">
          <div className="flex items-center justify-between mb-1">
      ...
      <div className="bg-green-50/50 rounded-xl border border-green-100 flex flex-col overflow-hidden h-full">
        <div className="p-4 border-b border-green-100 bg-white/50 rounded-t-xl shrink-0">
          <div className="flex items-center justify-between mb-1">
      ...
      {/* Approved */}
      <div className="bg-purple-50/50 rounded-xl border border-purple-100 flex flex-col overflow-hidden h-full">
        <div className="p-4 border-b border-purple-100 bg-white/50 rounded-t-xl shrink-0">
          <div className="flex items-center justify-between mb-1">
