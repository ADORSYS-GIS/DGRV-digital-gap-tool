...
export const AssessmentSubmissionList: React.FC<AssessmentSubmissionListProps> = ({
  organizationId,
}) => {
  const { data: submissions, isLoading, error } = useSubmissionsByOrganization(organizationId);
  ...
  return (
    <div className="grid gap-4">
      {submissions?.length === 0 && <p>No assessment submissions found for this organization.</p>}
        ...
        <Card key={submission.assessment.assessment_id}>
          ...
          <CardContent className="flex justify-between items-center">
            <div>
              <p className="text-sm text-muted-foreground">
                ...
                {submission.assessment.completed_at
                  ? new Date(submission.assessment.completed_at).toLocaleDateString()
                  : 'N/A'}
              </p>
            </div>
            <div className="space-x-2">
              <Button
                variant="outline"
                onClick={() => handleExportReport(submission.assessment.assessment_id)}
                disabled={downloadReportMutation.isPending}
              >
                Export Report
              </Button>
              <Link to={`/admin/action-plans/${submission.assessment.assessment_id}`}>
