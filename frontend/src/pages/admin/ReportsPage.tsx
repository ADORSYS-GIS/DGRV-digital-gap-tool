    ...
    <div className="container mx-auto py-8">
      ...
      {selectedOrganization ? (
        <div>
          ...
          <h2 className="text-2xl font-semibold mb-4">Submissions for {selectedOrganization.name}</h2>
          <AssessmentSubmissionList organizationId={selectedOrganization.id} />
        </div>
      ) : (
        <OrganizationReportList onSelectOrganization={setSelectedOrganization} />
      )}
    </div>
  );
};
