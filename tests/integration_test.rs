...
// Import from the local crate using the lib.rs module structure - using the crate name from Cargo.toml
use dgat_backend::config::MinioConfig;
use dgat_backend::entities::reports::{ReportFormat, ReportStatus, ReportType};
...
async fn test_s3_storage_service_creation() {
    ...
    assert!(
        result.is_ok(),
        "S3StorageService should be created successfully"
    );
}
...
async fn test_file_upload_download() {
    ...
    assert!(
        uploaded_path.contains(&config.bucket_name),
        "Upload path should contain bucket name"
    );
...
async fn test_report_generation_and_storage() {
    ...
    let report_service = ReportService::new(&config, std::sync::Arc::new(db)).await.unwrap();
    ...
    assert!(
        result.is_ok(),
        "Report generation and storage should succeed"
    );
    ...
    let retrieval_result = report_service.get_report_file(report.report_id).await;
